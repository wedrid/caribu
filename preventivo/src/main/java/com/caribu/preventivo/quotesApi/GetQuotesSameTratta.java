...
public class GetQuotesSameTratta implements Handler<RoutingContext> {
    DecimalFormat df = new DecimalFormat("#.###");
    private static final Logger LOG = LoggerFactory.getLogger(GetQuotesSameTratta.class);
    private final Pool db;
    private String string_query;

    public GetQuotesSameTratta(final Pool db) {
        this.db = db;
    }
                ...
                .doOnSuccess(result -> {
                    ...
                    result.forEach(row -> {
                        responseStr.append("\t Distanza richiesta: " + convertNum(row, "dist_in"))
                                ...
                                .append(", idPreventivo: " + row.getValue("id_quotes"))
                                .append(". \n");
                    });
                    LOG.info("ddd", result);
